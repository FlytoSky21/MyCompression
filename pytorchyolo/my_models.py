...
def create_modules(module_defs: List[dict]) -> Tuple[dict, nn.ModuleList]:
    ...
    for module_i, module_def in enumerate(module_defs):
        ...
        if module_def["type"] == "convolutional":
            ...
            filters = output_filters[0:][int(module_def["from"])]
...
def create_back_modules(module_defs: List[dict]) -> Tuple[dict, nn.ModuleList]:
    """
    Constructs module list of layer blocks from module configuration in module_defs
...
class DarknetFront(nn.Module):
    """YOLOv3 object detection model"""

    def __init__(self, config_path):
        super(DarknetFront, self).__init__()
        self.module_defs = parse_model_config(config_path)
        self.hyperparams, self.module_list = create_modules(self.module_defs)
        self.yolo_layers = [layer[0]
                            for layer in self.module_list if isinstance(layer[0], YOLOLayer)]
...
class DarknetBack(nn.Module):
    """YOLOv3 object detection model"""

    def __init__(self, config_path):
        super(DarknetBack, self).__init__()
        self.module_defs = parse_model_config(config_path)
        self.hyperparams, self.module_list = create_back_modules(self.module_defs)
        self.yolo_layers = [layer[0]
                            for layer in self.module_list if isinstance(layer[0], YOLOLayer)]
        self.seen = 0
        self.header_info = np.array([0, 0, 0, self.seen, 0], dtype=np.int32)

    def forward(self, x, imgg_size):
        layer_outputs, yolo_outputs = [], []
        layer_outputs.append(x)
        for i, (module_def, module) in enumerate(zip(self.module_defs, self.module_list)):
            if module_def["type"] in ["convolutional", "upsample", "maxpool"]:
...
def load_front_model(model_path, weights_path=None):
    """Loads the yolo model from file.
    ...
    model = DarknetFront(model_path).to(device)
...
def load_back_model(model_path, weights_path=None):
    """Loads the yolo model from file.
    ...
    model = DarknetBack(model_path).to(device)
