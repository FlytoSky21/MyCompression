...
def parse_args(argv):
    ...
    parser.add_argument("--checkpoint", default='/home/adminroot/taofei/MyCompression/result/grad_joint_training/flicker/concate_rrdb/0.0483/checkpoint_300.pth.tar', type=str,
...
def main(argv):
    ...
    for img_name in img_list:
        ...
        x_lr_edge = transforms.ToTensor()(edge_img_lr).unsqueeze(0).to(device)
        count += 1
        with torch.no_grad():
            s = time.time()
            out_net = net.forward(x, x_lr, x_lr_edge)
            e = time.time()
            total_time += (e - s)
