...
class DCC2023Model(CompressionModel):
    def __init__(self, Cs=256, N=192, M1=64, M2=128, M=192):
        ...
        self.f_block = RRDB(6, kernel_size=3, gc=32, stride=1, bias=True, pad_type='zero', \
    ...
    def forward(self, x, x_lr, x_lr_edge):
        ...
        grad_fea = torch.cat([x, x_grad], dim=1)
        # grad_fea = x + x_grad
        grad_fea = self.f_block(grad_fea)
        y2 = self.gx_a(grad_fea)
